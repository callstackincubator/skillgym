...
describe("max steps monitor", () => {
  test("counts codex steps on completed agent messages", () => {
    ...
    ).toMatchObject({ observedSteps: 2, maxSteps: 1 });
  });
  ...
  test("deduplicates claude assistant messages by id", () => {
    const monitor = createMaxStepsMonitor({
      ...
      runnerId: "claude-main",
      maxSteps: 1,
    });

    expect(monitor.observeLine('{"type":"assistant","message":{"id":"msg_1"}}')).toBeUndefined();
    expect(monitor.observeLine('{"type":"assistant","message":{"id":"msg_1"}}')).toBeUndefined();
    expect(monitor.observeLine('{"type":"assistant","message":{"id":"msg_2"}}')).toMatchObject({
      observedSteps: 2,
    });
  });
  ...
  test("counts cursor tool rounds and final assistant rounds", () => {
    const monitor = createMaxStepsMonitor({
      ...
      runnerId: "cursor-main",
      maxSteps: 1,
    });

    expect(
      monitor.observeLine('{"type":"tool_call","subtype":"started","model_call_id":"call-1"}'),
    ).toBeUndefined();
    expect(
      monitor.observeLine('{"type":"tool_call","subtype":"completed","model_call_id":"call-1"}'),
    ).toBeUndefined();
    expect(
